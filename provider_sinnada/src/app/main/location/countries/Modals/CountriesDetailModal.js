...
function CountriesDetailModal({item, open, closeHandler}) {
    const dispatch = useDispatch();
    const countryInfo = useSelector(({LocationApp}) => LocationApp.country.countryInfo);
    const loading = useSelector(({LocationApp}) => LocationApp.country.countryInfoLoading);
    ...
    useEffect(() => {
        if(item){
            dispatch(Actions.loadingCountryInfo(true));
            dispatch(Actions.getCountryInfo(item.id));
        }
    }, [item]);

    useEffect(() => {
        if(countryInfo){
            setValues(countryInfo);
        }
    }, [countryInfo]);
    ...
    function handleSubmit() {
        dispatch(Actions.loadingCountryInfo(true));
        dispatch(Actions.updateCountryItem(values));
        closeHandler();
    }
...
export default CountriesDetailModal;