...
export function getPackProducts(pack_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/pack_products/${pack_id}`, {
            params: filters
        });
...
export function removeFromPackProducts(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/catalog/pack_products/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getPackProductInfo(pack_product_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/pack_products/show/${pack_product_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
...
export function createPackProduct(item){
    return (dispatch, getState) => {
        const page = getState().CatalogApp.pack_product.page;
        const pack_id = getState().CatalogApp.pack_product.pack_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/catalog/pack_products/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getPackProducts(pack_id, page));
...
//         const page = getState().CatalogApp.pack_product.page;
//         const pack_id = getState().CatalogApp.pack_product.pack_id;
//         const request = axios.post(`${AUTH_CONFIG.url}/api/provider/catalog/pack_products/update`, {...item});
//         request.then((response) => {
//             if(response.data.code === 200){
//                 dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
//                 dispatch(getPackProducts(pack_id, page));
...
export function getProviderProducts(provider_id) {
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/products/provider/json/${provider_id}`);
        request.then((response) => {
                dispatch({
