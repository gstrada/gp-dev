...
export function getMethods(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/payment/methods`, {
            params: filters
        });
...
export function removeFromMethods(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/payment/methods/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getMethodInfo(method_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/payment/methods/${method_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: METHODS_INFO,
...
export function createMethod(item){
    return (dispatch, getState) => {
        const page = getState().PaymentApp.method.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/payment/methods/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getMethods(page));
...
export function updateMethodItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().PaymentApp.method.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/payment/methods/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getMethods(page));
