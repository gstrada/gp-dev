...
function StatesDetailModal({carrier_id, item, open, closeHandler}) {
    ...
    const loading = useSelector(({ShippingApp}) => ShippingApp.state.stateInfoLoading);
    ...
    if (!stateInfo || !carrierInfo || loading || !values) {
        return (
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true}  fullScreen={fullScreen}  maxWidth='sm'>
            <DialogTitle id="form-dialog-title">Editar una provincia de {carrierInfo.name}</DialogTitle>
