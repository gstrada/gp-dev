...
function CitiesCreateModal({carrier_state_id, open, closeHandler}) {


    const dispatch = useDispatch();
    const stateInfo = useSelector(({ShippingApp}) => ShippingApp.state.stateInfo);
    const loading = useSelector(({ShippingApp}) => ShippingApp.city.cityCreateLoading);

    const states = useSelector(({ShippingApp}) => ShippingApp.city.states);
    const state_cities = useSelector(({ShippingApp}) => ShippingApp.city.state_cities);

    const theme = useTheme();
    const fullScreen = useMediaQuery(theme.breakpoints.down('sm'));
    const [values, setValues] = React.useState({'carrier_state_id' : carrier_state_id});
    ...
    useEffect(() => {
        if(carrier_state_id){
            dispatch(Actions.getStateInfo(carrier_state_id));
        }
    }, [carrier_state_id]);
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true}  fullScreen={fullScreen}  maxWidth='sm'>
            <DialogTitle id="form-dialog-title">Agregá una ciudad a {stateInfo.carrier.name} ({stateInfo.state.name})</DialogTitle>
