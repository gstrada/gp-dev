...
export function getProducts(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/products`, {
            params: filters
        });
...
export function removeFromProducts(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/catalog/products/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getProductInfo(product_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/products/show/${product_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: PRODUCTS_INFO,
...
export function createProduct(item){
    return (dispatch, getState) => {
        const page = getState().CatalogApp.product.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/catalog/products/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateProductItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().CatalogApp.product.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/catalog/products/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateProductPicture(logoFile, id) {
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/catalog/products/picture`, form_data, {
            headers: {
