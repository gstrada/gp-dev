...
export function getCategories(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/categories`, {
            params: filters
        });
...
export function removeFromCategories(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/catalog/categories/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getCategoryInfo(category_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/catalog/categories/show/${category_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
...
export function createCategory(item){
    return (dispatch, getState) => {
        const page = getState().CatalogApp.category.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/catalog/categories/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateCategoryItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().CatalogApp.category.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/catalog/categories/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateCategoryPicture(logoFile, id) {
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/catalog/categories/picture`, form_data, {
            headers: {
