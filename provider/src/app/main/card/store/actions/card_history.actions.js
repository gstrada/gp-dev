...
export function getCardHistories(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/card/histories`, {
            params: filters
        });
...
export function removeFromCardHistories(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/card/histories/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getCardHistoryInfo(card_history_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/card/histories/show/${card_history_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
...
export function createCardHistory(item){
    return (dispatch, getState) => {
        const page = getState().CatalogApp.card_history.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/card/histories/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateCardHistoryItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().CatalogApp.card_history.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/card/histories/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
