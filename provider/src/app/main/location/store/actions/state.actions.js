...
export function getStates(country_id, page) {
    ...
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/location/states/${country_id}`, {
            params: filters
        });
...
export function removeFromStates(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/location/states/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getStateInfo(state_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/location/states/show/${state_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: STATES_INFO,
...
export function createState(item){
    return (dispatch, getState) => {
        const page = getState().LocationApp.state.page;
        const country_id = getState().LocationApp.state.country_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/location/states/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getStates(country_id, page));
...
export function updateStateItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().LocationApp.state.page;
        const country_id = getState().LocationApp.state.country_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/location/states/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getStates(country_id, page));
