...
function StatesCreateModal({country_id, open, closeHandler}) {
    const dispatch = useDispatch();
    const countryInfo = useSelector(({LocationApp}) => LocationApp.country.countryInfo);
    const loading = useSelector(({LocationApp}) => LocationApp.state.stateCreateLoading);
    const theme = useTheme();
    const fullScreen = useMediaQuery(theme.breakpoints.down('sm'));
    const [values, setValues] = React.useState({'country_id' : country_id});

    useEffect(() => {
        if(country_id){
            dispatch(Actions.getCountryInfo(country_id));
        }
    }, [country_id]);
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true}  fullScreen={fullScreen}  maxWidth='sm'>
            <DialogTitle id="form-dialog-title">Agregá una provincia a {countryInfo.name}</DialogTitle>
