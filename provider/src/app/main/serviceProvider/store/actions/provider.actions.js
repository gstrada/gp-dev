...
export function getProviders(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/service_provider/providers`, {
            params: filters
        });
...
export function removeFromProviders(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/service_provider/providers/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getProviderInfo(provider_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/service_provider/providers/show/${provider_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: PROVIDERS_INFO,
...
export function createProvider(item){
    return (dispatch, getState) => {
        const page = getState().ProviderApp.provider.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/service_provider/providers/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateProviderItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().ProviderApp.provider.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/service_provider/providers/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updateProviderLogo(logoFile, id) {
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/service_provider/providers/logo`, form_data, {
            headers: {
