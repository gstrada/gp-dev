...
export function getAddresses(provider_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/service_provider/addresses/${provider_id}`, {
            params: filters
        });
...
export function removeFromAddresses(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/provider/service_provider/addresses/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getAddressInfo(address_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/provider/service_provider/addresses/show/${address_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: ADDRESSES_INFO,
...
export function createAddress(item){
    return (dispatch, getState) => {
        const page = getState().ProviderApp.address.page;
        const provider_id = getState().ProviderApp.address.provider_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/service_provider/addresses/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getAddresses(provider_id, page));
...
export function updateAddressItem(item){
    return (dispatch, getState) => {
        const page = getState().ProviderApp.address.page;
        const provider_id = getState().ProviderApp.address.provider_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/provider/service_provider/addresses/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getAddresses(provider_id, page));
