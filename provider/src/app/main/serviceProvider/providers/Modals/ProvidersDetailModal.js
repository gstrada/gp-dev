...
function ProvidersDetailModal({item, open, closeHandler}) {

    let logoInput = useRef(null);

    const dispatch = useDispatch();
    const providerInfo = useSelector(({ProviderApp}) => ProviderApp.provider.providerInfo);
    const loading = useSelector(({ProviderApp}) => ProviderApp.provider.providerInfoLoading);
    ...
    useEffect(() => {
        if(item){
            dispatch(Actions.loadingProviderInfo(true));
            dispatch(Actions.getProviderInfo(item.id));
        }
    }, [item]);

    useEffect(() => {
        if(providerInfo){
            setValues(providerInfo);
        }
    }, [providerInfo]);
    ...
    function handleSubmit() {
        dispatch(Actions.loadingProviderInfo(true));
        dispatch(Actions.updateProviderItem(values));
        closeHandler();
    }
    ...
    if (!providerInfo || loading || !values) {
        return (
...
export default ProvidersDetailModal;