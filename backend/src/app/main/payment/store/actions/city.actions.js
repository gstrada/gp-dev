...
export function getCities(payment_method_state_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/payment/cities/${payment_method_state_id}`, {
            params: filters
        });
...
export function removeFromCities(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/payment/cities/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getCityInfo(city_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/payment/cities/show/${city_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: CITIES_INFO,
...
export function createCity(item){
    return (dispatch, getState) => {
        const page = getState().PaymentApp.city.page;
        const payment_method_state_id = getState().PaymentApp.city.payment_method_state_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/payment/cities/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCities(payment_method_state_id, page));
...
export function updateCityItem(item){
    return (dispatch, getState) => {
        const page = getState().PaymentApp.city.page;
        const payment_method_state_id = getState().PaymentApp.city.payment_method_state_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/payment/cities/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCities(payment_method_state_id, page));
