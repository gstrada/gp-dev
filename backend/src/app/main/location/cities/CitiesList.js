...
function CitiesList({openDeleteAlertHandler, openDetailModalHandler, state_id}) {

    const dispatch = useDispatch();
    const catalog = useSelector(({LocationApp}) => LocationApp.city.entities);
    const page = useSelector(({LocationApp}) => LocationApp.city.page);
    const loading = useSelector(({LocationApp}) => LocationApp.city.loading);

    useEffect(() => {
        dispatch(Actions.loadingCities(true));
        dispatch(Actions.getCities(state_id, page));
    }, [state_id]);
    ...
    function fetchData(city, table) {
        let table_page = city.page + 1;
        let new_page = {...page}
        new_page.current_page = table_page;
        //dispatch(Actions.getCountries(new_page));
        dispatch(Actions.getCities(state_id, new_page));
