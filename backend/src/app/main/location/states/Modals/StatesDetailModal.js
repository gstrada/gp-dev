...
function StatesDetailModal({item, open, closeHandler}) {
    ...
    const loading = useSelector(({LocationApp}) => LocationApp.state.stateInfoLoading);
    ...
    useEffect(() => {
        if(item){
            ...
            dispatch(Actions.getCountryInfo(item.country_id));
        }
    }, [item]);
    ...
    if (!stateInfo || !countryInfo || loading || !values) {
        return (
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true}  fullScreen={fullScreen}  maxWidth='sm'>
            <DialogTitle id="form-dialog-title">Editar una provincia de {countryInfo.name}</DialogTitle>
