...
export function getCountries(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/location/countries`, {
            params: filters
        });
...
export function removeFromCountries(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/location/countries/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getCountryInfo(country_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/location/countries/show/${country_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: COUNTRIES_INFO,
...
export function createCountry(item){
    return (dispatch, getState) => {
        const page = getState().LocationApp.country.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/location/countries/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCountries(page));
...
export function updateCountryItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().LocationApp.country.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/location/countries/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCountries(page));
