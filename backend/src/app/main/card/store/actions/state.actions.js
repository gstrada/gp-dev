...
export function getStates(payment_payment_method_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/payment/states/${payment_payment_method_id}`, {
            params: filters
        });
...
export function removeFromStates(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/payment/states/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getStateInfo(state_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/payment/states/show/${state_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: STATES_INFO,
...
export function createState(item){
    return (dispatch, getState) => {
        const page = getState().PaymentApp.state.page;
        const payment_method_id = getState().PaymentApp.state.payment_method_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/payment/states/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getStates(payment_method_id, page));
...
export function updateStateItem(item){
    return (dispatch, getState) => {
        const page = getState().PaymentApp.state.page;
        const payment_method_id = getState().PaymentApp.state.payment_method_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/payment/states/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getStates(payment_method_id, page));
