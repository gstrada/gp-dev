...
export function getProductPictures(product_id) {
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/catalog/product_picture/${product_id}`);
...
export function removeFromProductPictures(item){
    return (dispatch, getState) => {
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/catalog/product_picture/remove`, {
            id: item.id
        });
        request.then((response) => {
...
export function createProductPicture(item ) {
    ...
    return (dispatch) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/catalog/product_picture/store`, form_data, {
            headers: {
