...
export function getPacks(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/catalog/packs`, {
            params: filters
        });
...
export function removeFromPacks(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/catalog/packs/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getPackInfo(pack_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/catalog/packs/show/${pack_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: PACKS_INFO,
...
export function createPack(item){
    return (dispatch, getState) => {
        const page = getState().CatalogApp.pack.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/catalog/packs/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updatePackItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().CatalogApp.pack.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/catalog/packs/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                if(item.logo){
...
export function updatePackPicture(logoFile, id) {
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/catalog/packs/picture`, form_data, {
            headers: {
