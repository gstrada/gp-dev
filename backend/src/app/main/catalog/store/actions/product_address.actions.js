...
export function getProductAddresses(product_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/catalog/product_addresses/${product_id}`, {
            params: filters
        });
...
export function removeFromProductAddresses(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/catalog/product_addresses/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getProductAddressInfo(product_addresses_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/catalog/product_addresses/show/${product_addresses_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
...
export function createProductAddress(item){
    return (dispatch, getState) => {
        const page = getState().CatalogApp.product_address.page;
        const product_id = getState().CatalogApp.product_address.product_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/catalog/product_addresses/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getProductAddresses(product_id, page));
...
//         const page = getState().CatalogApp.product_address.page;
//         const product_id = getState().CatalogApp.product_address.product_id;
//         const request = axios.post(`${AUTH_CONFIG.url}/api/backend/catalog/product_addresses/update`, {...item});
//         request.then((response) => {
//             if(response.data.code === 200){
//                 dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
//                 dispatch(getProductAddresses(product_id, page));
...
export function getProviderAddresses(provider_id) {
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/service_provider/addresses/provider/json/${provider_id}`);
        request.then((response) => {
                dispatch({
