...
export function getUsers(provider_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/service_provider/users/${provider_id}`, {
            params: filters
        });
...
export function removeFromUsers(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/service_provider/users/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function createUser(item){
    return (dispatch, getState) => {
        const page = getState().ProviderApp.address.page;
        const provider_id = getState().ProviderApp.address.provider_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/service_provider/users/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getUsers(provider_id, page));
