...
export function getStates(shipping_carrier_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/shipping/states/${shipping_carrier_id}`, {
            params: filters
        });
...
export function removeFromStates(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/shipping/states/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getStateInfo(state_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/shipping/states/show/${state_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: STATES_INFO,
...
export function createState(item){
    return (dispatch, getState) => {
        const page = getState().ShippingApp.state.page;
        const carrier_id = getState().ShippingApp.state.carrier_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/shipping/states/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getStates(carrier_id, page));
...
export function updateStateItem(item){
    return (dispatch, getState) => {
        const page = getState().ShippingApp.state.page;
        const carrier_id = getState().ShippingApp.state.carrier_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/shipping/states/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getStates(carrier_id, page));
