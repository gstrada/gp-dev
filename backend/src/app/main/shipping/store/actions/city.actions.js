...
export function getCities(shipping_carrier_state_id, page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/shipping/cities/${shipping_carrier_state_id}`, {
            params: filters
        });
...
export function removeFromCities(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/shipping/cities/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getCityInfo(city_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/shipping/cities/show/${city_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: CITIES_INFO,
...
export function createCity(item){
    return (dispatch, getState) => {
        const page = getState().ShippingApp.city.page;
        const carrier_state_id = getState().ShippingApp.city.carrier_state_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/shipping/cities/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCities(carrier_state_id, page));
...
export function updateCityItem(item){
    return (dispatch, getState) => {
        const page = getState().ShippingApp.city.page;
        const carrier_state_id = getState().ShippingApp.city.carrier_state_id;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/shipping/cities/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCities(carrier_state_id, page));
