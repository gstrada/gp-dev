...
export function getCarriers(page) {
    return (dispatch) => {
        ...
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/shipping/carriers`, {
            params: filters
        });
...
export function removeFromCarriers(item_id){
    return (dispatch, getState) => {
        ...
        const request = axios.post( `${AUTH_CONFIG.url}/api/backend/shipping/carriers/remove`, {
            id: item_id
        });
        request.then((response) => {
...
export function getCarrierInfo(carrier_id){
    return (dispatch) => {
        const request = axios.get(`${AUTH_CONFIG.url}/api/backend/shipping/carriers/${carrier_id}`);
        request.then((response) => {
            if(response.data.code === 200){
                dispatch({
                    type: CARRIERS_INFO,
...
export function createCarrier(item){
    return (dispatch, getState) => {
        const page = getState().ShippingApp.carrier.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/shipping/carriers/store`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCarriers(page));
...
export function updateCarrierItem(item){
    ...
    return (dispatch, getState) => {
        const page = getState().ShippingApp.carrier.page;
        const request = axios.post(`${AUTH_CONFIG.url}/api/backend/shipping/carriers/update`, {...item});
        request.then((response) => {
            if(response.data.code === 200){
                dispatch(Actions.showMessage({message: response.data.message, variant: 'success'}));
                dispatch(getCarriers(page));
